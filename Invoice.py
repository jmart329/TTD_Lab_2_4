class Invoice:
    ...
    #calculates tax using a fixed rate
    def totalTax(self, products):
        taxRate = .075
        total_tax = self.totalPurePrice(products)*taxRate
        return total_tax

    #calculates the final price by adding tax to total pure price
    def finalPrice(self, products):
        final_price = self.totalPurePrice(products)+self.totalTax(products)
